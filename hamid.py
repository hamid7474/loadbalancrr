...
frontend http_front
    bind *:${ports[0]} # اولین پورت برای ورودی
    default_backend http_back

backend http_back
    server target $target_server:${ports[0]}
EOF

    # اضافه کردن پورت‌های دیگر به کانفیگ
    for port in "${ports[@]:1}"; do
        sudo bash -c "echo '    bind *:$port' >> /etc/haproxy/haproxy.cfg"
        sudo bash -c "echo '    server target $target_server:$port' >> /etc/haproxy/haproxy.cfg"
    done
...
create_gre_tunnel() {
    echo -n "Enter Local IPv6 Address: "
    read local_ip
    echo -n "Enter Remote IPv6 Address: "
    read remote_ip
