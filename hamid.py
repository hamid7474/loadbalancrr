...
EOF
    ...
    # اضافه کردن پورت‌های ورودی به frontend
    for port in "${ports[@]}"; do
        sudo bash -c "echo '    bind *:$port' >> /etc/haproxy/haproxy.cfg"
    done

    # ادامه دادن به پیکربندی backend
    sudo bash -c "echo '    default_backend http_back' >> /etc/haproxy/haproxy.cfg"
    sudo bash -c "echo '' >> /etc/haproxy/haproxy.cfg"
    sudo bash -c "echo 'backend http_back' >> /etc/haproxy/haproxy.cfg"

    # اضافه کردن سرورهای backend برای هر پورت
    for port in "${ports[@]}"; do
